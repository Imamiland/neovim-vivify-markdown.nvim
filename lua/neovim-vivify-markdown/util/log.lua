...
function log.notify(scope, level, verbose, str, ...)
    if
        not verbose
        and _G.NeovimVivifyMarkdown.config ~= nil
        and not _G.NeovimVivifyMarkdown.config.debug
    then
        return
    end
    ...
    vim.notify(
        string.format("[neovim-vivify-markdown.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "neovim-vivify-markdown.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h NeovimVivifyMarkdown.options` to read more."
        )
    end
end
